...
int main( int argc, char** argv )
{
	...
	openMVG::voctree::VocabularyTree<FeatureFloat> tree ( treeName );
	...
	size_t numTotFeatures = 0; //@TODO port me!  popart::vision::database::populateDatabase( keylist, tree, db, documents );
	...
	for ( DocumentMap::const_iterator docIter = documents.begin( ), ie = documents.end( ); docIter != ie; ++docIter )
	{
		detect_start = std::chrono::steady_clock::now();
		db.find( docIter->second, numImageQuery, matches );
		...
		POPART_COUT( "query document " << docIter->first << " took "<< detect_elapsed.count() <<"sec and has " << matches.size( ) << " matches\tBest " << matches[0].id << " with score " << matches[0].score << endl );

		
		allMatches[ docIter->first ] = ListOfImageID( );
		allMatches[ docIter->first ].reserve( matches.size() );
		
		for( openMVG::voctree::Match m : matches )
		{
			allMatches[ docIter->first ].push_back( m.id );
		}
