...
int main( int argc, char** argv )
{
  ...
  {
    if ( !( Load( map_PutativesMatches, sOutputMatchesFilename ) ) )
    {
  ...
  else // Compute the putative matches
  {
    ...
    system::Timer timer;
    {
      ...
      if (cmd.used('P')) // Preemptive filter
      {
        ...
        std::swap(map_filtered_matches, map_PutativesMatches);
      }

      //---------------------------------------
      //-- Export putative matches & pairs
      //---------------------------------------
      if ( !Save( map_PutativesMatches, std::string( sOutputMatchesFilename ) ) )
      {
        OPENMVG_LOG_ERROR
