...
///
//- Create an Image_describer interface that use an OpenCV feature extraction method
// i.e. with the SURF detector+descriptor
//--/!\ If you use a new Regions type you define and register it in
//   "openMVG/features/regions_factory.hpp" file.
typedef features::AKAZE_Float_Regions SURF_OpenCV_Regions;
// Define the Interface
...
CEREAL_REGISTER_TYPE_WITH_NAME(SURF_OCV_Image_describer, "SURF_OCV_Image_describer");
...
//- Create an Image_describer interface that use an OpenCV feature extraction method
// i.e. with the ORB detector+descriptor
//--/!\ If you use a new Regions type you define and register it in
//   "openMVG/features/regions_factory.hpp" file.
typedef features::ORB_Binary_Regions ORB_OpenCV_Regions;
// Define the Interface
class ORB_OCV_Image_describer : public Image_describer
{
public:
  ORB_OCV_Image_describer():Image_describer(){}
  ...
  */
    ...
    const Image<unsigned char> * mask = nullptr)
  {
    ...
    if (!vec_keypoints.empty())
    {
      ...
      // Build alias to cached data
      ORB_OpenCV_Regions * regionsCasted = dynamic_cast<ORB_OpenCV_Regions*>(regions.get());
      // reserve some memory for faster keypoint saving
      regionsCasted->Features().reserve(vec_keypoints.size());
      regionsCasted->Descriptors().reserve(vec_keypoints.size());
  ...
  void Allocate(std::unique_ptr<Regions> &regions) const
  {
    regions.reset( new ORB_OpenCV_Regions );
  }
...
CEREAL_REGISTER_TYPE_WITH_NAME(ORB_OCV_Image_describer, "ORB_OCV_Image_describer");
...
int main(int argc, char **argv)
{
  ...
  else
  {
    ...
    if (sImage_Describer_Method == "ORB_OPENCV")
    {
      image_describer.reset(new ORB_OCV_Image_describer);
    }
    else
