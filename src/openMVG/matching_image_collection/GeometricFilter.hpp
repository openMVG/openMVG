...
/// *** VISIDYN SHIT: loads matches directly from mapped features **** ////
  ...
  void Filter(
    ...
    std::map<size_t, std::vector<FeatureT> >& map_Feats,
    PairWiseMatches & map_PutativesMatchesPair, // putative correspondences to filter
    PairWiseMatches & map_GeometricMatches,
    std::vector<std::pair<size_t, size_t> > & vec_imagesSize) const
  {
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    for (int i = 0; i < (int)map_PutativesMatchesPair.size(); ++i)
    {
      ...
      typename std::map<size_t, std::vector<FeatureT> >::const_iterator iterFeatsI = map_Feats.find(iIndex);

      typename std::map<size_t, std::vector<FeatureT> >::const_iterator iterFeatsJ = map_Feats.find(jIndex);
...
/// END VISIDYN SHIT!!!! YEAH!!!! ITS JEEZY!! AHAH!!! ///// 
  /// Filter all putative correspondences according the templated geometric filter
  template <typename GeometricFilterT>
  void Filter(
    const GeometricFilterT & geometricFilter,  // geometric filter functor
