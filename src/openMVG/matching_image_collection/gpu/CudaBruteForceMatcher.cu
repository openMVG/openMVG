...
__threadfence_block();
    ...
    for (int j=1; j<8; j<<=1) for (int i = 0; i < 4; ++i) dist[i] += __shfl_xor(dist[i], j);
    for (int i=1; i<4; ++i) if ((threadIdx.x & 3) == i) dist[0] = dist[i];
    dist[0] = __byte_perm(dist[0], 0, threadIdx.x & 4 ? 0x5432 : 0x5410);
