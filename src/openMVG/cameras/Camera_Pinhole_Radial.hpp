...
/**
 * @brief Implement a Pinhole camera with a 1 radial distortion coefficient for pba.
 * \f$ x_u = x_d (1 + K_1 r^2 ) \f$
 */
class Pinhole_Intrinsic_Radial_K1_PBA : public Pinhole_Intrinsic
{
  using class_type = Pinhole_Intrinsic_Radial_K1_PBA;
...
public:
  ...
  * @param k1 Distortion coefficient
  */
  Pinhole_Intrinsic_Radial_K1_PBA(
          int w = 0, int h = 0,
          double focal = 0.0, double ppx = 0, double ppy = 0,
          double k1 = 0.0 )
          : Pinhole_Intrinsic( w, h, focal, ppx, ppy ),
  ...
  ~Pinhole_Intrinsic_Radial_K1_PBA() override = default;
  ...
  */
  EINTRINSIC getType() const override
  {
    return PINHOLE_CAMERA_RADIAL1_PBA;
  }
  ...
  */
  bool updateFromParams( const std::vector<double> & params ) override
  {
    if ( params.size() == 4 )
    {
      *this = Pinhole_Intrinsic_Radial_K1_PBA(
              w_, h_,
              params[0], params[1], params[2], // focal, ppx, ppy
              params[3] ); //K1
      return true;
