...
class Pinhole_Intrinsic_Fisheye : public Pinhole_Intrinsic
{
    ...
    virtual Vec2 add_disto(const Vec2 & p) const{
        ...
        double inv_r = r > eps ? 1.0/r : 1;
        double cdist = r > eps ? theta_dist * inv_r : 1;
        return  p*cdist;
    }

    virtual Vec2 remove_disto(const Vec2 & p) const{
        double scale = 1.0;
        double theta_dist = std::sqrt(p[0]*p[0] + p[1]*p[1]);
        if (theta_dist > eps)
        {
            ...
            for(int j = 0; j < 10; j++ )
            {
                ...
                theta = theta_dist / (1 + _params[0] * theta2 + _params[1] * theta4 + _params[2] * theta6 + _params[3] * theta8);
            }
