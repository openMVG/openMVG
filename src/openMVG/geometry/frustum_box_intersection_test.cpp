...
TEST(frustum_box, intersection)
{
  ...
  // Test with truncated frustum
  {
    ...
    for (int i = 0; i < iNviews; ++i)
    {
      ...
      std::ostringstream os;
      os << i << "frust_truncated.ply";
      export_Ply(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i], d._C[i]), os.str());
    }

    const double radius = 4;
    for (double angle = 0; angle < 2 * M_PI; angle += M_PI / 12.)
    {
      Vec3 boxPosition = Vec3::Zero();
      boxPosition[0] = cos(angle) * radius - 1;
      boxPosition[1] = sin(angle) * radius - 4;

      Mat3 R = Mat3::Identity();
      R = Eigen::AngleAxis<double>(rand(), Vec3::UnitZ())
        *  Eigen::AngleAxis<double>(rand(), Vec3::UnitY())
        *  Eigen::AngleAxis<double>(rand(), Vec3::UnitZ());
      const Box box(boxPosition, sqrt(1.), R);

      std::ostringstream os;
      os << angle << "box.ply";
      //export_Ply(box, os.str());

      const Vec3 red(255, 0, 0), green(0, 255, 0);
      // Check that frustums have an overlap
      int bIntersect = 0;
      for (int i = 0; i < iNviews; ++i)
          bIntersect += vec_frustum[i].intersect(box) ? 1 : 0;
       export_Ply(box, os.str(), bIntersect==4 ? green : red);
    }
  }
}
*/
