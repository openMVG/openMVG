...
invert_intrinsics(
    ...
    const double px[2], double nrm[2])
{
...
// 
static void 
invert_intrinsics_tgt(const double K[/*3 or 2 ignoring last line*/][ncoords2d_h], const double pix_tgt_coords[][ncoords2d], double normalized_tgt_coords[][ncoords2d], unsigned npts)
{
  for (unsigned p=0; p < npts; ++p) {
    const double *tp = pix_tgt_coords[p];
...
static bool
read_block(std::istream &in, double *p, unsigned n)
{
  LOG("reading");
...
int main(int argc, char **argv) {
  ...
  if (!stdio_) {
    infp.open(intrinsics_filename, std::ios::in);
    if (!infp) {
      std::cerr << "I/O Error opening input " << intrinsics_filename << std::endl;
      return EXIT_FAILURE;
    }
    inp = &infp;
  }
  ...
  for (const auto &track_it: tracks)
  {
    ...
    invert_intrinsics(K, datum[0].col(idx), px_normalized);
    // orientation:
    // theta = feature_i.orientation();
    // orientation = (std::cos(theta), std::sin(theta))
    // 
    invert_intrinsics(K, orientation, orientation_normalized);

    theta_normalized = atan2(orientation_normalized[0], orientation_normalized[1]);

    datum[0].col(idx) << px_normalized[0], px_normalized[1], theta_normalized;
  ...
  invert_intrinsics(K, datum[0], datum[0], 3);
  
  invert_intrinsics(K, pix_coords[][ncoords2d], normalized_coords[][ncoords2d], npts);
  invert_intrinsics_tgt(K, pix_tgt_coords[][ncoords2d], normalized_tgt_coords[][ncoords2d], npts);
