              ...
              vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx)
{
    ...
    vector<int> cam_id;
    Hc.clear(); Hp.clear(); cam_id.clear();
    int np = 0, nc = 0;
    for(size_t i = 0; i < nproj; ++i)    
    {
        double x, y;    int cidx, pidx;
        in >> cidx >> pidx >> x >> y;
        cam_id.push_back(cidx);
        if(Hp.find(pidx) == Hp.end()) Hp[pidx] = np++;
        pidx = Hp[pidx];
    ...
    sort(cam_id.begin(), cam_id.end());
    for(auto x : cam_id) if(Hc.find(x) == Hc.end()) Hc[x] = nc++;
    for(auto &x : camidx) x = Hc[x];
    ...
    for(size_t i = 0; i < ncam; ++i)
    {
        ...
        in>>p[0];
        CameraT& cam = camera_data[i];
        for(int j = 0; j < 12; ++j) in >> p[j];
        for(int j = 0; j < 9; j++) cam.m[j/3][j%3] = p[j];
        for(int j = 9; j < 12; j++) cam.t[j-9] = p[j];
    }
              ...
              vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx)
{
    ...
    for(size_t i = 0; i < camera_data.size(); ++i)
    {
        CameraT& cam = camera_data[i]; out<<0<<"\n";
        for(int j = 0; j < 9; j++) out<<cam.m[j/3][j%3]<<" ";
        for(int j = 0; j < 3; j++) out<<cam.t[j]<<" "; out<<"\n";
